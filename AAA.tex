...
Since we know that $\sum_{i=1}^n x_{Ni} = 0$, we can write
...
$\omega_i$ weight according to its size.
...
The variance will be interpreted as to what level respondents agreement about Belgian economy.

?This different from the sampling error. The idea here is not the study accuracy or ...
but rather to find new information hidden in the survey.
...
We can now do the same for the weighted indicator. The equation is very similar;
...
Variance is a little bit more complex to 
...
%In the BSIndicator, this is not taken into account. To show this lets take a simple example; let imagine that from a certain month to the other one, the BSI is the same, it will be interpreted as no changes in companies view. This is one explanation, but it is possible that 

This means that we can see the individual evolution of each of the individual participants. A working paper \cite{caron_estimation_1996}, written by researchers at l'INSEE, proposed an indicator that take into account the responses at the previous survey and gave value to the evolution.
