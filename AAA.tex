...
In order to make it possible to have a long period of time to analyse, the decision is made to take only unweighted indicators for the empirical analysis since the weights are not available before 2008. The weighting procedure been very complex and prone to some companies influencing largely the indicator. Also the weights are only available for the period after 2008, here the dataset obtained goes from 1988 to 2018.

For similar reasons, the BSI is obtained only for the industrial sector.
It is the first sector that was taken into account in the business survey before it gets enlarged to Trade, .. and Services that was only included recently in the calculation of the published general indicator.
