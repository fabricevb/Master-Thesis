...
\begin{equation}
    \mbox{Weighted Industry BSI}\ = \frac{\sum_{i=1}^n \omega \left(x_{i Q1} + x_{i Q2} + x_{i Q3} + x_{i Q4} \right)}{4}
\end{equation}
...
Before we model the data, it's important to look at the different effects that could mislead the outcome of the analysis. After exploring the data and doing a literature review, there are four main effects that could mislead the results. Three are due to the survey and one to external effects; seasonal effects. We will first look into seasonal effects and apply a well know correction to the data to eliminate, as good as possible, the seasonal effects from the data.
...
Eight variables will be used for the analysis. First the BSI, Var(BSI), .......
...
The literature about seasonal effects is very rich and variate. Without going to much into details, there are today two methods which are the most widely used and recommended which are X12/X13-ARIMA (XXXXX) developed by the US Census Bureau and TRAMO/SEATS (Gómez y Maravall, 1996) developed by the Bank of Spain. The two methods are divide in two part, first ..........
...
Test for seasonality where done with JDemetra+ (see appendix \autopageref{sec:test for seasonality}) and it was concluded that there was seasonal effect for each of the variables at hand (see \autoref{tab:Seasonality Tests})
...
It's interesting here to notice that there is no need for Year on Year GDP of been seasonally corrected, since by definition, it's eliminating all it's seasonality.
...
The solution applied in case of non-response is assuming that the respondent would have answered the same as previous month, the method is called "Last Observation Carried Forward" (LOCF).
...
Those two issues are related to the structure and organisation of the survey. As explain in \autoref{sec:Recruitment of participants}, ones participants are recruited to participate in the business survey, they stay as long as they want. This brings two potential issues: dropout and attrition.
...
This is an issue, since it means that it's a certain type of companies that leave the survey. If this type of profile have a different opinion or respond differently than the remaining companies this will create bias. 
...
An interesting way to explore attrition, is by looking at correlation of the variance over time. 
In \autoref{tab:corr attrition} it can be seen that 

Same can be observed if we look at the evolution of the variance over time in \autoref{fig:seasonal ajusted rjdemetra} where, aside of the peak of variance during the economical crises of 2008, there is a general tendency of decreasing of 

It would be very interesting to dive more into deep of this issues, but this will be left for further research. In the context of this paper, it's important to notice the potential importance of attrition and dropout.
...
In a second time it will be looked into the correlations among the questions taken into account in the .......................
...
There are three different correlations that need to be looked at; (1) between questions of the industry BSI, (2) between YoY GDP and the different indicators and there variance and (3) the autocorrelation.
...
The interest of this chapter is to use a model to test the pertinence of the variance of the indicator, the evolution of the individual responses and it variance, in the short term prediction of the evolution of GDP.
...
Better model could be used; Linear AR models, ARIMA, State Space Models, ... but here interest is to ...

The interest of this paper is to study the variance of the business survey indicator and explore the possible interest of the indicator of the evolution in individual responses. To achieve this objective, it's important to chose a model that account easily for the interest of each variable. For those reasons, the Linear model is applied here.

\section{Method}

The quarterly GDP and the Quarterly Year on Year GDP is set at the last month of the quarter. F
