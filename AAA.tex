...
...............In a second part, correlations will be looked at; (1) correlation among the questions indicators (taken into account in the calculation of the industry BSI) and with the YoY GDP. (2) 
...
The different variables are unweighted and seasonally corrected. 
The data is taken over a 30 years period, from 1988 to 2018.

The different variables at hand are summarised in \autoref{tab:descriptive statistics} and there time series are plotted in appendix, on \autopageref{plot:variables}.

The Year on Year GDP calculation was explained in \autoref{eq:YoY GDP}. It can be seen that it has a quite cyclical behaviour, with lower point during the 1994 and 2008 crisis.

- BSI
The business survey indicator seems, from \autoref{plot:variables}, to follow quite well the evolution of YoY GDP. The other variables, seem to react to 2008 crises but are more volatile. It seems from those plots that the BSI is the most accurate
...
\autoref{fig:corr indicators} shows the correlation between Year on Year GDP and the different variables at hand. 

- corr GDP  indicators

The correlation between the business survey indicator and YoY GDP is very high while the other variables have a rather small correlation with YoY GDP.


- corr BSI \& var

Interesting to note is the quite large correlation between the indicator and the variance of the

- corr EIR(1 2 3) \& var(EIR1 2 3)

- corr BSI \& EIR

- corr EIR \& var(EIR)

- corr EIR(1 2 3) var(EIR1 2 3) \& corr EIR(1 2 3) var(EIR1 2 3)
...
To achieve this objective, it's important to use a model that account easily for the interest of each variable. For those reasons, the linear model is applied here.
...
\begin{tabular}{l l}
    $YoY \text{ } GDP_t$     & Year on Year GDP at time t \\
