...
% Zotero Bialowolski 2014

% https://ec.europa.eu/eurostat/cros/system/files/Jdemetra_%20release.pdf
...
The survey has the particularity of being a panel survey. The new indicator of EIR takes advantage of it. The information of the evolution of each participant is compared to its previous answer.
...
De nieuwe indicator voor de evolutie van de individuele antwoorden maakt er gebruik van. Het antwoord van elke deelnemer wordt vergeleken met die van het maand ervoor.
...
This thesis is part of a tradition of research proposing improvement and ways to add value to surveys. Different methods will be proposed to add information to the Belgian business survey indicator (BSI), that could also be applied to others similar surveys.
...
Chapter 7 test for seasonal effects and explain the seasonal correction applied to the data. It will then discuss some potential issues/bias of the survey; non-responses, dropout and attrition.
In Chapter 8, descriptive statistics will be made of the different variables at hand and it will be looked into correlations among the different variables.
...
Lot of other propositions were done even though business cycles are rather empirically defined than theory based.
...
The most significant changes brought in 2009 were the number of questions taken into the calculation of the BSI, which was reduced to 3-4 questions, depending on the sector, for more simplicity and accuracy.
...
The second is obtained by measuring the importance in the economy of a certain sub-division of it. In other words, what will be refered to here as the globalisation weight, is the weight given to a specific sub-sector according to its importance in Belgian economy.
The procedure developed by the National Bank of Belgium to obtain the globalisation weight, is an elaborate division of the Belgian economic activity. This means that for example, the industry is subdivided into different sub-sectors, that them self contain sub-sectors that contain sub-sectors and so on for a total of six levels. 
Each division has a percentage according to its size in the economy.
To obtain the weight of the lowest globalisation, the weight of each subdivision need to be multiplied. 
...
where $\sum_{i=1}^{n} \omega_i = 1 $. In other words, the specific weight for each company are taking the two weighting procedures into account, the weight specific to the size of the company, and the weight specific to the sub-sector the company is part of.
...
The method can be written as follow
...
As described in \autoref{sec:Weighting procedure}, each respondent has two different weighting procedures: one according to its size, one according to the size of the sector it's part of. Those weights are then combined and end up with a specific weight $\omega_i$.
...
See appendix \autopageref{chap:appendix:explain Var BSI and EIR} for further interpretation.
...
As explained in the chapter two - that presented the business survey - the business survey is answered by the same companies over time. Some new participants join and some companies leave the survey, but the survey can be referred to and treated as a panel survey.
...
The same as for the business survey indicator, the evolution of individual responses take different possible values. In the case of the BSI, $x_i$ can take value -1, 0 and 1. In the case of the EIR, $z_i$ can take values -2, -1, 0, 1 and 2.
...
Further explanation including examples in order to better understand the different indicators and variances, can be found in Appendix on \autopageref{chap:appendix:explain Var BSI and EIR}.
...
This chapter will explain those different specificities and, to some extend, explain the different reasons while it's the data used in this paper.
...
Another reason not to use weighted measures is that the weighting procedure is very complex and prone to some companies influencing largely the indicator, and with it the results of the research. 


\section{Period}

The period covered is from 1988 to 2018. This 30 years long period was chosen based on the electronic availability of the data and the interest of having a long period of time including several business cycles.
...
the third questions relate to the company's predictions of need of work forces and the last question ask their predictions regarding demand.
...
The layout was modified, and the phrasing of the questions slightly evolved over this long period. It was before asked in the first person while it's now phrased in the third person. Aside from those small changes, the survey kept the same questions and order.
...
The year on year gross domestic product (YoY GDP) which is publicly available.

The eight other variables are obtained with the different calculation explained in the first part of this paper.
...
EIR1 is when taking the difference in answer between a month and the month before while EIR2 uses a lag of two months and EIR3 a lag of three months. The reasons for taking a larger period into account where explained in \autoref{sec:EIR lg longer period into account}.

For each of the three measures of the EIR, the variance is calculated and will further be studied.
...
One is due to external effects; seasonal effects
while four are due to the survey structure and organisation.
First it will be looked into seasonal effects and a well known correction to the data will be applied to eliminate - as good as possible - the seasonal effects from the data.
...
The seasonal-irregular (S-I) ratio charts that can be seen in appendix, on \autopageref{fig:S-I seasonal correction RJDemetra}, are interesting to look at to better understand the seasonal effect influencing the different variables.
...
% The non-response of the business survey can be decomposed into two parts; non-participation to the survey and participants not responding to the survey (non-monotone).
...
In the case of the business survey, the LOCF technique assumes that observations are missing completely at random. This method is not the most efficient and unbiased but has the advantage of been very simple and have more data (compared to considering the missing observations as missing). 
...
%The parameters of the test are forecast horizon = 1, loss function power = 2 and alternative hypothesis: two.sided and 
...
%The linear regression also showed that the indicator of the evolution of individual responses and its variance slightly improve the model. The interpretation of those that can be done is very interesting, those two indicators would be recommended to better understand how respondents respond in a survey.
