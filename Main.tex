...
% Evidence shows that industry is the sector with the highest correlation with GDP and is 
...
From \autoref{eq: BSI Unweighted} and \autoref{eq: BSI Weighted} it can be seen that the weighted and unweighted indicators are bounded between -1 and 1. In the two cases, the indicator is the smallest if all respondents have a negative answer, and is the largest when every answer is positive.
...
See \autoref{tab:values EIR} for the different values of $z$
...
In other words, the variance is quite similar to the variance of the business survey indicator. In this case there is the particularity that we have nine possible answers where three take value zero, two take value one, two take value minus one, one takes value two and another takes value minus two. 
...
\subsubsection{Property 1: the variance of Z is bounded between 0 and 4}
...
Multiple Imputations would be an interesting approach to handle non-response in the business survey. It could decrease the bias and increase accuracy. 
...
Markov Switching, State-Space Models, MIDAS ....
...
\href{https://github.com/fabricevb/Master-Thesis/blob/master/R Code/Linear Regression.R}{https://github.com/fabricevbMaster-Thesis/blob/master/R Code/Linear Regression.R}
